...
export function NotificationsPage() {
  ...
  const segmentOptions = [...new Set(analytics.flatMap((item) => item.segments))];
  const visibleCoupons = coupons.filter((coupon) => {
    const stats = analytics.find((item) => item.couponId === coupon.id);
    const expired = Boolean((coupon.scadenza && coupon.scadenza < today) || (coupon.dataAppuntamento && coupon.dataAppuntamento < today));
    const matchesStatus = couponStatus === "tutti"
      || (couponStatus === "attivi" && coupon.attivo && !expired)
      || (couponStatus === "scaduti" && expired);
    return (!couponSearch.trim() || coupon.codice.toLowerCase().includes(couponSearch.trim().toLowerCase()))
      && matchesStatus
      && (!couponSegment || stats?.segments.includes(couponSegment));
  });
  ...
  return (
    <section>
      ...
      <section className="coupon-performance-summary"><article><span>Migliore conversione</span><strong>{bestCoupon?.conversionRate ?? 0}%</strong><small>{coupons.find((coupon) => coupon.id === bestCoupon?.couponId)?.codice ?? "Nessun invio"}</small></article><article><span>Tempo medio di risposta</span><strong>{averageSpeed || "—"}{averageSpeed ? " gg" : ""}</strong><small>dall’invio all’utilizzo</small></article><article><span>Coupon monitorati</span><strong>{analytics.length}</strong><small>{analytics.reduce((sum, item) => sum + item.utilizzati, 0)} utilizzi complessivi</small></article></section>
      <div className="coupon-filters"><label><span>Cerca coupon</span><input value={couponSearch} onChange={(event) => setCouponSearch(event.target.value)} placeholder="Codice coupon" /></label><label><span>Stato</span><select value={couponStatus} onChange={(event) => setCouponStatus(event.target.value as "tutti" | "attivi" | "scaduti")}><option value="tutti">Tutti</option><option value="attivi">Attivi</option><option value="scaduti">Scaduti</option></select></label><label><span>Segmento</span><select value={couponSegment} onChange={(event) => setCouponSegment(event.target.value)}><option value="">Tutti i segmenti</option>{segmentOptions.map((segment) => <option key={segment}>{segment}</option>)}</select></label></div>
      {visibleCoupons.map((c) => (
        <article className={`coupon-monitor-card ${c.attivo ? "" : "is-inactive"}`} key={c.id}>
          ...
          {(() => { const stats = analytics.find((item) => item.couponId === c.id); const max = Math.max(...(stats?.redemptionSeries.map((item) => item.count) ?? [1]), 1); const phaseMax = Math.max(stats?.usagePhases.iniziale ?? 0, stats?.usagePhases.centrale ?? 0, stats?.usagePhases.finale ?? 0, 1); return <><div className="coupon-metrics">{(["inviati", "utilizzati", "nonUtilizzati", "scaduti"] as const).map((kind) => <button type="button" onClick={() => { setMetricSearch(""); setOpenMetric({ coupon: c, kind }); }} key={kind}><b>{stats?.[kind] ?? 0}</b>{kind === "nonUtilizzati" ? "Non utilizzati" : kind.slice(0, 1).toUpperCase() + kind.slice(1)}</button>)}</div><div className="coupon-analytics-row"><div className="coupon-redemption-trend"><span>Utilizzi nelle ultime 8 settimane</span><div>{(stats?.redemptionSeries ?? []).map((point) => <i title={`${point.label}: ${point.count}`} style={{ height: `${Math.max(6, point.count / max * 100)}%` }} key={point.label} />)}</div></div><div className="coupon-phases"><span>Quando viene usato</span><div>{(["iniziale", "centrale", "finale"] as const).map((phase) => <i key={phase}><b style={{ width: `${((stats?.usagePhases[phase] ?? 0) / phaseMax) * 100}%` }} /><small>{phase} · {stats?.usagePhases[phase] ?? 0}</small></i>)}</div></div><dl><div><dt>Conversione</dt><dd>{stats?.conversionRate ?? 0}%</dd></div><div><dt>Tempo medio</dt><dd>{stats?.averageDaysToUse || "—"}{stats?.averageDaysToUse ? " gg" : ""}</dd></div></dl></div></>; })()}
          <button className="btn btn--ghost" type="button" onClick={() => void toggleCoupon(c)}>{c.attivo ? "Disattiva" : "Riattiva"}</button>
        </article>
      ))}
      {coupons.length > 0 && visibleCoupons.length === 0 && <div className="coupon-filter-empty"><strong>Nessun coupon trovato</strong><span>Modifica ricerca, stato o segmento.</span></div>}
      ...
      {openMetric && (() => { const stats = analytics.find((item) => item.couponId === openMetric.coupon.id); const ids = openMetric.kind === "utilizzati" ? stats?.usedClientIds ?? [] : openMetric.kind === "nonUtilizzati" || openMetric.kind === "scaduti" ? (stats?.recipientIds ?? []).filter((id) => !stats?.usedClientIds.includes(id)) : stats?.recipientIds ?? []; const people = ids.map((id) => clients.find((client) => client.id === id)).filter(Boolean) as SalonClient[]; const query = metricSearch.trim().toLowerCase(); const filteredPeople = query ? people.filter((person) => `${person.nome} ${person.email} ${person.telefono ?? ""}`.toLowerCase().includes(query)) : people; return <div className="dashboard-modal" role="dialog" aria-modal="true"><button type="button" aria-label="Chiudi" onClick={() => setOpenMetric(null)} /><section><header><div><span>{openMetric.coupon.codice}</span><h3>{openMetric.kind === "nonUtilizzati" ? "Non utilizzati" : openMetric.kind}</h3><small>{people.length} clienti</small></div><button type="button" onClick={() => setOpenMetric(null)}>×</button></header><label className="dashboard-modal__search"><span>Cerca nella lista</span><input value={metricSearch} onChange={(event) => setMetricSearch(event.target.value)} placeholder="Nome, email o telefono" autoFocus /></label>{filteredPeople.length ? filteredPeople.map((person) => <article key={person.id}><div><strong>{person.nome}</strong><span>{person.email || person.telefono || "Contatto non disponibile"}</span></div>{stats?.usedAtByClient[person.id] && <time>{new Intl.DateTimeFormat("it-IT", { day: "numeric", month: "short", year: "numeric" }).format(new Date(stats.usedAtByClient[person.id]))}</time>}</article>) : <p>{people.length ? "Nessun cliente corrisponde alla ricerca." : "Nessun cliente in questa categoria."}</p>}</section></div>; })()}
    </section>
  );
}
