    ...
    <section>
      ...
      {coupons.map((c) => (
        <article className={`coupon-monitor-card ${c.attivo ? "" : "is-inactive"}`} key={c.id}>
          ...
          {(() => { const stats = analytics.find((item) => item.couponId === c.id); const max = Math.max(...(stats?.redemptionSeries.map((item) => item.count) ?? [1]), 1); return <><div className="coupon-metrics">{(["inviati", "utilizzati", "nonUtilizzati", "scaduti"] as const).map((kind) => <button type="button" onClick={() => { setMetricSearch(""); setOpenMetric({ coupon: c, kind }); }} key={kind}><b>{stats?.[kind] ?? 0}</b>{kind === "nonUtilizzati" ? "Non utilizzati" : kind.slice(0, 1).toUpperCase() + kind.slice(1)}</button>)}</div><div className="coupon-redemption-trend"><span>Utilizzi nelle ultime 8 settimane</span><div>{(stats?.redemptionSeries ?? []).map((point) => <i title={`${point.label}: ${point.count}`} style={{ height: `${Math.max(6, point.count / max * 100)}%` }} key={point.label} />)}</div></div></>; })()}
