...
const formatDay = (value: string, options: Intl.DateTimeFormatOptions) => new Intl.DateTimeFormat("it-IT", options).format(parseDate(value));
...
export function BookingsPage() {
  ...
  const confirmedCount = dayBookings.filter((booking) => booking.stato === "confermata").length;
  const completedCount = dayBookings.filter((booking) => booking.stato === "completata").length;
  const isToday = anchor === dateKey(new Date());
  const countForDate = (date: string) => active.filter((booking) => booking.date === date).length;
  ...
  return <section className="agenda-page">
    <header className="dashboard-page-header"><div><span>Agenda</span><h2>Calendario</h2><p>Giornata, carico e richieste in un’unica vista.</p></div></header>
    {notice && <p className="owner-home__notice" role="status">{notice}</p>}
    <section className="agenda-context-bar" aria-label="Riepilogo della giornata selezionata">
      <div className="agenda-context-bar__date"><span>{isToday ? "Oggi" : formatDay(anchor, { weekday: "long" })}</span><strong>{formatDay(anchor, { day: "numeric", month: "long" })}</strong></div>
      <div className="agenda-context-bar__total"><strong>{dayBookings.length}</strong><span>{dayBookings.length === 1 ? "prenotazione" : "prenotazioni"}</span></div>
      <dl className="agenda-context-bar__metrics"><div><dt>Confermate</dt><dd>{confirmedCount}</dd></div><div><dt>Completate</dt><dd>{completedCount}</dd></div><div className={pendingCount ? "has-pending" : ""}><dt>Da gestire</dt><dd>{pendingCount}</dd></div></dl>
    </section>
    ...
    <div className={`agenda-workspace${view === "settimana" ? " is-week" : ""}`}>
      <div className={`agenda-calendar ${view === "giorno" ? "is-operator-view" : "is-date-view"}`} style={{ "--agenda-columns": calendarColumnCount } as CSSProperties}>
        <div className="agenda-calendar__head"><span>Ora</span>{view === "giorno" ? dayColumns.map((operator) => <div className="agenda-operator-head" aria-label={`Agenda di ${operator.nome}`} key={operator.id}>{operator.fotoUrl ? <img src={operator.fotoUrl} alt={`Foto di ${operator.nome}`} /> : <span aria-hidden="true">{initials(operator.nome) || "—"}</span>}<strong>{operator.nome}</strong><small>{dayBookings.filter((booking) => booking.operatorId === operator.id).length} appuntamenti</small></div>) : visibleDates.map((date) => <button className={date === anchor ? "is-active" : ""} type="button" onClick={() => setAnchor(date)} key={date}><small>{formatDay(date, { weekday: "short" })}</small><strong>{parseDate(date).getDate()}</strong><span>{countForDate(date)} app.</span></button>)}</div>
