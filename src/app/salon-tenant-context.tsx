...
interface SalonTenantState {
  ...
  error: string | null;
  selectSalon: (salon: SalonWithId) => void;
  clearSalonSelection: () => void;
}

const STORAGE_KEY = "salon-suite:selected-salon";
...
export function SalonTenantProvider({ children }: { children: ReactNode }) {
  ...
  const [state, setState] = useState<SalonTenantState>({
    loading: true,
    salon: null,
    error: null,
    selectSalon,
  ...
  function selectSalon(salon: SalonWithId) {
    window.localStorage.setItem(STORAGE_KEY, salon.id);
    setState({
      loading: false,
      salon,
      error: null,
  ...
  useEffect(() => {
    ...
    async function resolveTenant() {
      try {
        ...
        const previewId = isLocal
          ? new URLSearchParams(window.location.search).get("salon")?.trim()
          : "";
        const selectedId = window.localStorage.getItem(STORAGE_KEY)?.trim();
        const configuredId =
          accountSalonId ||
          previewId ||
          selectedId ||
          import.meta.env.VITE_SALON_ID?.trim() ||
          (isLocal ? "salone-x" : "");
        if (configuredId) {
          if (active)
            setState((current) =>
              current.salon?.id === configuredId
                ? current
                : {
                    loading: true,
                    salon: null,
        ...
        if (active)
          setState({
            loading: false,
            salon: null,
            error: "Questo salone non è configurato.",
            selectSalon,
            clearSalonSelection,
          });
      }
  ...
  }, [accountSalonId, authLoading, selectionVersion]);

  return (
    <SalonTenantContext.Provider value={state}>
      {children}
    </SalonTenantContext.Provider>
  );
}
...
export function useSalonTenant(): SalonTenantState {
  ...
  if (!value)
    throw new Error(
      "useSalonTenant deve essere usato dentro SalonTenantProvider",
    );
  return value;
}
